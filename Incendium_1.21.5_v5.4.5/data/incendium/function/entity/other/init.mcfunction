...
#> Castle
# elytra
kill @s[type=item,nbt={Item:{components:{"minecraft:custom_data":{incendium:{item:'paratrooper_wings'}}}}}]

# powder
execute if entity @s[type=item,nbt={Item:{components:{"minecraft:custom_data":{incendium:{item:"unstable_powder"}}}}}] run function incendium:item/unstable_powder/init

# alchemist
data modify entity @s[type=splash_potion,tag=in.alchemist_potion] NoGravity set value 0b
