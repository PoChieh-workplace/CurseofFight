...
execute if score $rand in.dummy matches 1.. run tellraw @s {translate:"incendium.item.returning_scroll.system.withstood",fallback:"The ancient technique withstood itself, this time...", "italic": true, "color": "#D393F5"}
...
execute unless score $rand in.dummy matches 1.. run tellraw @s {translate:"incendium.item.returning_scroll.system.crumble",fallback:"The ancient technique crumbled upon usage", "italic": true, "color": "#D393F5"}